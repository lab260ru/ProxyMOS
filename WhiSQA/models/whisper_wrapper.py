...
def mel_filters(device, n_mels: int = 80) -> torch.Tensor:
    ...
    # путь к текущей папке (MOS_RES/models/)
    base_dir = os.path.dirname(os.path.abspath(__file__))
    mel_path = os.path.join(base_dir, "mel_filters.npz")

    if not os.path.exists(mel_path):
        raise FileNotFoundError(f"Не найден mel_filters.npz по пути: {mel_path}")

    with np.load(mel_path, allow_pickle=True) as f:
