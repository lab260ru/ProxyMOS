...
class MOSNet(nn.Module):
    ...
    def getFtrMaps(self, forward_input):
        ...
        # FIX: Use .real and .imag attributes instead of indexing
        real = input_stft.real  # ✅ Extract real part
        imag = input_stft.imag  # ✅ Extract imaginary part
        
        input_stft = torch.sqrt(torch.clamp(real ** 2 + imag ** 2, min=1e-7)).float()
        input_stft = input_stft[..., 2:-2].unsqueeze(1).permute((0, 1, 3, 2))
